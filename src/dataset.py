...
def _parse_image_function(example):
    ...
    shape = tf.stack([1, image_size, image_size, -1, 1], axis=0)
...
def _get_parsing_function(path):
...
def naked_dataset(path):
    dataset = tf.data.TFRecordDataset(path)
    return dataset.map(_get_parsing_function(path))
...
if __name__ == "__main__":
    ...
    dataset = dataset.map(_get_parsing_function(args.path))
